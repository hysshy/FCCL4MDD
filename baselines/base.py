...
from utils.toolkit import tensor2numpy, grouped_accuracy, cal_forget_measure
from utils.data_manager import partition_data, DatasetSplit
...
class BaseLearner(object):
    ...
    def init_dataloader(self, data_manager, syn_data_manager):
        if syn_data_manager is not None:
            fl_syn_train_dataset = syn_data_manager.get_dataset(
                np.arange(0, self._total_classes),
                source="train",
                mode="train",
            )
            self.fl_syn_train_loader = DataLoader(
                fl_syn_train_dataset, batch_size=128, shuffle=True, num_workers=4
            )

            if self._known_classes > 0:
                cl_syn_train_dataset = syn_data_manager.get_dataset(
                    np.arange(0, self._known_classes),
                    source="train",
                    mode="train",
                )
                self.cl_syn_train_loader = DataLoader(
                    cl_syn_train_dataset, batch_size=128, shuffle=True, num_workers=4
                )
        ...
        for idx in range(len(user_groups)):
            if self._known_classes > 0:
                if syn_data_manager is not None:
                    self.local_train_loaders.append(DataLoader(ConcatDataset([cl_syn_train_dataset, DatasetSplit(train_dataset, user_groups[idx])]),
                        batch_size=self.args["local_bs"], shuffle=True, num_workers=4))
                else:
                    self.local_train_loaders.append(DataLoader(DatasetSplit(train_dataset, user_groups[idx]),
                        batch_size=self.args["local_bs"], shuffle=True, num_workers=4))
            else:
                split_dataset = DatasetSplit(train_dataset, user_groups[idx])
                self.local_train_dataset.append(split_dataset)
                self.local_train_loaders.append(DataLoader(split_dataset,
                        batch_size=self.args["local_bs"], shuffle=True, num_workers=4))
